...
def post_process_detections(image_np, boxes, labels, scores, 
                             footer_ratio=0.06, overlap_iou_thresh=0.05):
    """
    ...
    # Step 2: Content-aware box tightening
    ...
    # Step 3: Cross-class overlap resolution
...
def _tighten_boxes(image_np, boxes, labels):
    ...
    for box, lbl in zip(boxes, labels):
        ...
        if lbl == 1:  # PartDrawing
            refined = _tighten_partdrawing(roi, x1, y1, x2, y2, gray, h, w)
        elif lbl == 2:  # Note
...
def _tighten_note(roi, x1, y1, x2, y2):
    ...
    # Add small padding (3% of dimensions)
    pad_x = max(3, int(bw * 0.03))
    ...
    # Don't over-tighten for notes (max 15% per side)
    max_trim = 0.15
    new_x1 = max(new_x1, x1 + int(bw * 0) )  # Allow full left tighten for notes
...
def _tighten_table(roi, x1, y1, x2, y2):
    ...
    # Find first/last strong horizontal lines (table borders)
    h_line_rows = np.where(h_proj > bw * 0.3)[0]
    v_line_cols = np.where(v_proj > bh * 0.3)[0]
...
def _resolve_cross_class_overlap(boxes, labels, scores, iou_thresh=0.05):
